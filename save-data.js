...
const MAX_PROJECTS = 200;          // obyektlar soni
...
function sanitize(input, cfg, files, errors) {
  ...
  if (projects.length > MAX_PROJECTS) errors.push("Obyektlar soni juda ko'p.");
  const seenPr = new Set();
  projects.slice(0, MAX_PROJECTS).forEach((p, i) => {
    if (!p || typeof p !== "object") return;
    let id = str(p.id, 64).replace(/[^\w-]/g, "") || `o${i}`;
    while (seenPr.has(id)) id += "x";
    seenPr.add(id);

    const imgs = (Array.isArray(p.img) ? p.img : (p.img ? [p.img] : []))
      .slice(0, MAX_PROJECT_IMAGES)
      .map((x) => takeImage(x, files, cfg, errors))
      .filter(Boolean);
    ...
    const item = {
      ...
      products: (Array.isArray(p.products) ? p.products : [])
        .slice(0, 40).map((x) => str(x, 64).replace(/[^\w-]/g, "")).filter(Boolean)
    };
    for (const l of ["uz", "ru", "en"]) {
      const t = p[l] && typeof p[l] === "object" ? p[l] : {};
      item[l] = {
        n: str(t.n, 300),                                  // obyekt nomi
        c: str(t.c, 200),                                  // buyurtmachi
        l: str(t.l, 200),                                  // joylashuvi
        d: str(t.d, 2000),                                 // izoh
        s: (Array.isArray(t.s) ? t.s : []).slice(0, 40).map((x) => str(x, 250)).filter(Boolean)
      };
    }
    if (!item.uz.n && !item.ru.n && !item.en.n) return;
...
export default async function handler(req, res) {
  ...
  try {
    const msg = `admin: kontent yangilandi (${clean.products.length} mahsulot, ${clean.projects.length} obyekt${files.length ? `, ${files.length} rasm` : ""})`;
